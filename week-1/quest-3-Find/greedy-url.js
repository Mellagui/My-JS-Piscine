...
const rgx = /(http|https)\:{1}\/{2}[\w\-\,\%\°\[\]\.\/\?\=\&]+\s*?/gi;
const subRgxAnd = /&*/g;
const subRgxEqual = /=*/g;

const getURL = (dataSet) => dataSet.match(rgx)? dataSet.match(rgx): [];

const greedyQuery = (dataSet) => {
  let arr = getURL(dataSet); 
  let res = []; 
  arr.map((url) => {
  if (url.match(subRgxAnd).join('').length >= 2) res.push(url)}); return res || []
};

const notSoGreedy = (dataSet) => {
  let arr = getURL(dataSet); let res = []; arr.map((url) => { 
  if (url.match(subRgxEqual).join('').length >= 2 && url.match(subRgxAnd).join('').length <= 2) res.push(url)}); return res || []   
};
